...
shared_var = "I am the parent"

def child_process(shared_var):
    shared_var = "I am the child"
    print(f"[Child] shared_var = {shared_var}")

if __name__ == "__main__":
    print(f"[Parent] Before process: shared_var = {shared_var}")

    p = multiprocessing.Process(target=child_process, args=tuple([shared_var]))
    p.start()
    p.join()

    print(f"[Parent] After process: shared_var = {shared_var}")
